    ...
    odfi::language::nx::new [namespace current] {
        ...
        +type NameDescription : ::odfi::attributes::AttributesContainer  {
            ...
            +method getFullName args {
               ...
               return [:shade { if {[$it isClass ::odfi::NameDescription] && ![$it isClass ::odfi::Odfi]} { return true} else {return false} } formatHierarchyString {
                   if {[$it isClass ::odfi::Config]} {
                       return "config:[$it name get]"
                   }   elseif {[$it isClass ::odfi::Module] && [$it isPhysical]} {
                         return "[file tail [$it directory get]]"
                    } elseif {[$it isClass ::odfi::NameDescription]} {
                        return "[$it name get]"
                    } else {
                       return ""
                    }
           ...
           +method getFullHierarchyName args {
        ...
        +type Indexing {
            ...
            +method getValueAs {name script} {

                ## Search in Value Index.
                ## If found, return otherwise use script to build, save and return
                set nameIndex [lsearch -index 0 -exact ${:valueIndex} $name]
                if {$nameIndex==-1} {

                    set value [uplevel $script]
                    lappend :valueIndex [list $name $value]

                    return $value
                } else {
                    return [lindex ${:valueIndex} [list $nameIndex 1]]
                }
        ...
        :odfi : NameDescription name {
            ...
            ## Commands
            ########################
            +method getAllCommands args {

                set commands [::odfi::flist::MutableList new]
                :shade ::odfi::Command walkDepthFirstLevelOrder {
            ...
            :config : NameDescription configFile {
                ...
                :module : NameDescription name {
                    ...
                    ## For physical, use version for real full name
                    ## For other modules, keep it as usual
                    +method getFullName args {
                        if {![:isPhysical]} {
                            return [next]
                        } else {
                            set res [:getFullHierarchyName]
                            set res $res/[file tail [:directory get]]
                            return $res
                        }                        
                    }
                    ...
                    ## Reset is used to remove definitions from module description 
                    ## used for example if module file needs to be reloaded
                    +method reset args {
                        :shade ::odfi::powerbuild::Requirements eachChild {$it detach}
                    }
                    +method reloadDirectory args {
                        :reset
                        :useDirectory [:getDirectory]
                    }
                    +method useDirectory moduleDirectory {
                        ...
                        if {[::odfi::Version::isVersion [file tail ${:directory}]]} {
                            ...
                            ## Change name to match parent folder
                            set :name [file tail [file normalize ${:directory}/../]]
                            
                            ## Add version
                            :version [file tail ${:directory}]
                        }
                    ...
                    +method getVersion args {
                        if {$args!=""} {
                            ...
                            return [:shade ::odfi::Version findChildByProperty value "$args"]
                    ...
                    :version : NameDescription value {
                        ...
                        +objectMethod isVersion v {
                            if {[file tail ${v}]=="dev" || [file tail ${v}]=="master" || [regexp ${::odfi::Version::versionRegexp} $v]} {
                                return true
                            } else {
                                return false
                            }
                    ...
                    +type Runnable : NameDescription {
                        ...
                        +method wait args {
                            set :waitVariable true
                            ${:commandResult} setState vwait
                            
                            #after idle "if {\[[current object] waitVariable get\]==false} { [current object] waitVariable set true  } else {}"
                            
                            #::update
                            vwait :waitVariable
                            #::update
                            ${:commandResult} setState post-vwait
                            :log:raw "Stop listening"
                        }
                        
                        +method notify args {
                            :log:raw "Notifying [current object]"
                            ::set :waitVariable false
                        ...
                        ## Run command from a command. Look in uppath then call on main odfi if not found
                        ## Format: command -> look in uppath
                        ## Format: path/to/command -> use odfi global code
                        +method runCommand {path args} {

                            #puts "Local runcommand: $path -> $args"

                            set splited [split $path /]
                            if {[llength $splited]>1} {
                                return [[:getODFI ] runCommand $path [join $args]]
                            } else {

                                ## Look into parent
                                set parent [:findParentInPrimaryLine {$it shade ::odfi::Runnable findChildByProperty name $path}]
                                if {$parent!=""} {
                                    set command [$parent  shade ::odfi::Runnable findChildByProperty name $path]
                                    return [$command run [join $args]]
                                } else {
                                    return [[:getODFI ] runCommand $path [join $args]]
                                }
                        ...
                        +method runCommandGet {path args} {

                            ::set r [:runCommand $path [join $args]]
                            return [$r get]
                        }
                    ...
                    :command : Runnable name script {
                        ...
                        :commandResult {
                            ...
                            +method getMapContent args {
                                return [dict get ${:values}]
                            }
                            
                            +method clean args {
                                :clearChildren
                                set :values [dict create] 
                                set :stream [::new ::odfi::richstream::RichStream #auto]
                            }
            ...
            +method findModules {modulePath {version ""}} {
                ...
                if {[string match "config:*" [lindex $modulePath 0]]} {
                ...
                ## Select Configs
                if {$targetConfigObject!=""} {
                    $searchConfigs += $targetConfigObject
                } else {
                    :shade ::odfi::Config eachChild {
                        $searchConfigs += $it
                    }
                        ...
                        while {[llength $searchModulePath]>0} {
                           ...
                           #puts "Looking at $currentSearchPath -> version=[::odfi::Version::isVersion $currentSearchPath]"
                            ...
                            if {[llength $searchModulePath]==0 && [::odfi::Version::isVersion $currentSearchPath]} {
                                ...
                                #puts "Looking for module with version $currentSearchPath"
                                set foundChildRes [[$currentNode shade ::odfi::Module children] findOption {
                                
                                    puts "Testing [$it name get] -> [$it getVersion] -> [[$it getVersion] value get]"
                                    if {[$it shade ::odfi::Version findChildByProperty value $currentSearchPath]!=""} {
                                       # puts "Found"
                                        return true
                                    } else {
                                        return false
                                ...
                                if {[llength $searchModulePath]==0} {
                                   ...
                                   # puts "Using subtree search"
                                    ...
                                    set foundChild [$currentNode shade ::odfi::Module findFirstInTreeLevelOrder {
                                        if {[$it info class]!="::odfi::repo::Module" && [$it name get]==$currentSearchPath} {
                                            return true
                                        } else {
                                            return false
                                        }
                            ...
                            if {$foundChild==""} {
                                ...
                                #puts "Found [$currentNode info class] for $currentSearchPath, search path length is: [llength $searchModulePath] "
                ...
                if {[file exists $mainCommand] && ![file isdirectory $mainCommand]} {
                    ...
                    $foundModules isEmpty {
                        ...
                        if {$module==""} {
                            ...
                            :log:debug "Using the first returned module: $module"
